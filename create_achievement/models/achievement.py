...
class Achievement(models.Model):
	...
	computed_numbers = fields.Integer('Cái này chỉ để chạy hàm thôi', compute='_compute_numbers', store=True)
	...
	def _compute_numbers(self):
		for record in self:
			...
			for group_criteria in record.criteria_ids:
				group_criteria.display_name = str(number) + '. ' + group_criteria.name
				number += 1
